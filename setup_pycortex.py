...
parser.add_argument('--experiment-short', type=str, default='', 'shorthand experiment name to prepend to pycortex subject id')
...
import_subj('sub-{:02d}'.format(args.subnum), freesurfer_subject_dir=bids_dir+'/derivatives/freesurfer', sname=pyc_sname)

# # then import flattened surfaces from freesurfer
if args.import_flat:
    try:
        import_flat(subject, 'full', sname=pyc_sname)
...
shutil.copyfile(bids_dir+'/derivatives/freesurfer/{}/mri/T1w.cortex.nii'.format(subject),
    '{}/{}/anatomicals/T1w.cortex.nii'.format(config.get('basic', 'filestore'), pyc_sname))

# # create the xfm transformation
sub_dir = bids_dir+'/derivatives/freesurfer/{}'.format(subject)
os.system('tcsh fsreg2xfm {}/mri/register.dat {}/mri/T1w.cortex.nii {} {} standard'.format(sub_dir, sub_dir, pyc_sname, subject))
